...
type UnderlineNavProps<T extends { type: string }> = {
  items: T[];
  id: string;
  renderNavigationHeader: (header: T["type"]) => ReactNode;
  renderView: (view: T) => ReactNode;
  initialActiveIndex?: number;
  canRender?: boolean;
};
const UnderlineNav = <T extends { type: string }>({
  items,
  id,
  ...
  const navigationItemsRefs = useRef<HTMLDivElement[] | null[]>([]);
  ...
  useEffect(() => {
    if (canRender && activeIndex !== -1) {
      (
        navigationItemsRefs.current[activeIndex] as HTMLDivElement
      ).scrollIntoView({
        behavior: "smooth",
        inline: "center",
        block: "center",
