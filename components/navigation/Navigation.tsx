...
const Navigation = () => {
  ...
  const [isNavigationLoaded, setIsNavigationLoaded] = useState<boolean>(false);
  ...
  useEffect(() => {
    if (!hasCookie("theme-mode")) {
      ...
      dispatch(changeTheme(getCookie("theme-mode") as ThemeTypes));
    }
  }, []);
  ...
  return (
    <motion.nav
      ...
      className={`fixed top-0 inset-x-0 flex h-[4rem] items-center justify-between gap-4 z-[100] border-b-1 backdrop-blur-sm shadow-sm ${layoutProperties.padding} ${theme.foreground} ${theme.backgroundDark}`}
    >
